...
class hr_indicadores_previsionales(models.Model):
    ...
    fonasa = fields.Float('Fonasa', readonly=True, states=STATES, help="Fonasa", default=7.0)
    mutual_seguridad = fields.Float(
        'Mutualidad', readonly=True, states=STATES, help="Mutual de Seguridad")
    isl = fields.Float(
        'ISL', readonly=True, states=STATES, help="Instituto de Seguridad Laboral",default=0.93)
    ...
    def update_document(self):
        ...
        try:
            ...
            def clear_leftof(op,cad):
                if op in cad:
                    cad = cad[cad.find(op)+len(op):]
                return cad
        ...
        def index_first_table():
            return


        try:
            # 0 UF
            rows= letters[0].tbody.find_all('tr')
            #self.uf = clear_string(letters[0].select("strong")[0].get_text())
            self.uf = clear_string(rows[1].select('td')[1].get_text())

            # 1 UTM y UTA
            rows= letters[1].tbody.find_all('tr')
            # self.utm = clear_string(letters[1].select("strong")[0].get_text())
            # self.uta = clear_string(letters[1].select("strong")[1].get_text())
            self.utm = clear_string(rows[1].select('td')[1].get_text())
            self.uta = clear_string(rows[1].select('td')[2].get_text())

            # 2 RENTAS TOPES IMPONIBLES UF
            rows= letters[2].tbody.find_all('tr')
            # self.tope_imponible_afp = clear_string(letters[2].select("strong")[0].get_text())
            # self.tope_imponible_ips = clear_string(letters[2].select("strong")[1].get_text())
            # self.tope_imponible_seguro_cesantia = clear_string(letters[2].select("strong")[2].get_text())
            self.tope_imponible_afp = clear_string(rows[1].select('td')[1].get_text())
            self.tope_imponible_ips = clear_string(rows[2].select('td')[1].get_text())
            self.tope_imponible_seguro_cesantia = clear_string(rows[3].select('td')[1].get_text())

            # 3 RENTAS TOPES IMPONIBLES
            rows= letters[3].tbody.find_all('tr')
            ...
            self.sueldo_minimo = clear_string(rows[1].select('td')[1].get_text())
            self.sueldo_minimo_otro = clear_string(rows[2].select('td')[1].get_text())

            # 4 Ahorro Previsional Voluntario
            rows= letters[4].tbody.find_all('tr')
            # self.tope_mensual_apv = clear_string(letters[4].select("strong")[0].get_text())
            # self.tope_anual_apv = clear_string(letters[4].select("strong")[1].get_text())
            self.tope_mensual_apv = clear_string(rows[1].select('td')[1].get_text())
            self.tope_anual_apv = clear_string(rows[2].select('td')[1].get_text())

            # 5 DEPÓSITO CONVENIDO
            rows= letters[5].tbody.find_all('tr')
            # self.deposito_convenido = clear_string(letters[5].select("strong")[0].get_text())
            self.deposito_convenido = clear_string(rows[1].select('td')[1].get_text())

            # 6 RENTAS TOPES IMPONIBLES
            rows= letters[6].tbody.find_all('tr')
            ...
            self.contrato_plazo_indefinido_empleador = clear_string(rows[3].select('td')[1].get_text())
            self.contrato_plazo_indefinido_trabajador = clear_string(rows[3].select('td')[2].get_text())
            self.contrato_plazo_fijo_empleador = clear_string(rows[4].select('td')[1].get_text())
            self.contrato_plazo_indefinido_empleador_otro = clear_string(rows[5].select('td')[1].get_text())

            # 8 TASA COTIZACIÓN OBLIGATORIO AFP
            rows= letters[8].tbody.find_all('tr')
            ...
            self.tasa_afp_capital = clear_string(rows[4].select('td')[1].get_text())
            self.tasa_sis_capital = clear_string(rows[4].select('td')[2].get_text())

            # self.tasa_afp_cuprum = clear_string(letters[8].select("strong")[11].get_text())
            # self.tasa_sis_cuprum = clear_string(letters[8].select("strong")[12].get_text())
            self.tasa_afp_cuprum = clear_string(rows[5].select('td')[1].get_text())
            self.tasa_sis_cuprum = clear_string(rows[5].select('td')[2].get_text())
            ...
            self.tasa_afp_habitat = clear_string(rows[6].select('td')[1].get_text())
            self.tasa_sis_habitat = clear_string(rows[6].select('td')[2].get_text())
            ...
            self.tasa_afp_planvital = clear_string(rows[7].select('td')[1].get_text())
            self.tasa_sis_planvital = clear_string(rows[7].select('td')[2].get_text())

            # self.tasa_afp_provida = clear_string(letters[8].select("strong")[20].get_text())
            # self.tasa_sis_provida = clear_string(letters[8].select("strong")[21].get_text())
            self.tasa_afp_provida = clear_string(rows[8].select('td')[1].get_text())
            self.tasa_sis_provida = clear_string(rows[8].select('td')[2].get_text())
            ...
            self.tasa_afp_modelo = clear_string(rows[9].select('td')[1].get_text())
            self.tasa_sis_modelo = clear_string(rows[9].select('td')[2].get_text())
            ...
            self.tasa_afp_uno = clear_string(rows[10].select('td')[1].get_text())
            self.tasa_sis_uno = clear_string(rows[10].select('td')[2].get_text())

            # self.tasa_independiente_capital = clear_string(letters[8].select("strong")[10].get_text())
            self.tasa_independiente_capital = clear_string(rows[4].select('td')[3].get_text())
            self.tasa_independiente_cuprum = clear_string(rows[5].select('td')[3].get_text())
            self.tasa_independiente_habitat = clear_string(rows[6].select('td')[3].get_text())
            self.tasa_independiente_planvital = clear_string(rows[7].select('td')[3].get_text())
            self.tasa_independiente_provida = clear_string(rows[8].select('td')[3].get_text())
            self.tasa_independiente_modelo = clear_string(rows[9].select('td')[3].get_text())
            self.tasa_independiente_uno = clear_string(rows[10].select('td')[3].get_text())

            # 9 ASIGNACIÓN FAMILIAR

            rows= letters[9].tbody.find_all('tr')
            self.asignacion_familiar_monto_a = clear_string(rows[2].select('td')[1].get_text())
            self.asignacion_familiar_monto_b = clear_string(rows[3].select('td')[1].get_text())
            self.asignacion_familiar_monto_c = clear_string(rows[4].select('td')[1].get_text())

            self.asignacion_familiar_primer = clear_string(rows[2].select('td')[2].get_text())
            self.asignacion_familiar_segundo = clear_string(clear_leftof('< =',rows[3].select('td')[2].get_text()))
            self.asignacion_familiar_tercer = clear_string(clear_leftof('< =',rows[4].select('td')[2].get_text()))
