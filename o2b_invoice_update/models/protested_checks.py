...
class AccountChecksProtestedReport(models.AbstractModel):
    _name = "account.check.protested.report"
    ...
    def _get_templates(self):
        templates = super(AccountChecksProtestedReport, self)._get_templates()
    ...
    def _force_strict_range(self, options):
        ...
        previous_years = datetime.now() - relativedelta(years=6)
    ...
    def _get_options_domain(self, options):
        # OVERRIDE
        domain = super(AccountChecksProtestedReport, self)._get_options_domain(options)
    ...
    def _get_query_sums(self, options_list, expanded_account=None):
        ...
        if self._context and 'allowed_company_ids' in self._context and self._context.get('allowed_company_ids')\
            ...
            expanded_account = self.env['account.account'].browse(583)
            ...
            and self._context.get('allowed_company_ids')[0] == 2:
                expanded_account = self.env['account.account'].browse(1189)
        else:
        ...
        for i, options_period in enumerate(options_list):
            ...
            queries.append('''
                SELECT
                    ...
                    'sum'                                                   AS key,
                    MAX(COALESCE(account_move_line.date_maturity, account_move_line.date))                    AS max_date,
                    %s                                                      AS period_number,
