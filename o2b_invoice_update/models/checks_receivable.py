...
class AccountChecksReceivableReport(models.AbstractModel):
    _name = "account.check.receivable.report"
    _description = "Account Check Receivable Report"
    ...
    def _get_templates(self):
        templates = super(AccountChecksReceivableReport, self)._get_templates()
    ...
    def _get_general_ledger_lines(self, options, line_id=None):
        ...
        if not line_id:
            ...
            #     total_debit = cumulated_balance
            #     total_credit = 0
            #     total_balance= cumulated_balance
    ...
    def _force_strict_range(self, options):
        ...
        previous_years = datetime.now() - relativedelta(years=3)
        new_options.get('date').update({'string': 'Custom', 'period_type': 'custom', 'mode': 'range', 'strict_range': True, 'date_from': str(previous_years.date()), 'date_to': str(current_date), 'filter': 'custom'})
        return new_options

    @api.model
    def _get_options_domain(self, options):
        # OVERRIDE
        domain = super(AccountChecksReceivableReport, self)._get_options_domain(options)
    ...
    def _get_query_sums(self, options_list, expanded_account=None):
        ...
        # ============================================
        # Point:- 430
        if self._context and 'allowed_company_ids' in self._context and self._context.get('allowed_company_ids')\
            and self._context.get('allowed_company_ids')[0] == 1:
            expanded_account = self.env['account.account'].browse(391)
        elif self._context and 'allowed_company_ids' in self._context and self._context.get('allowed_company_ids')\
            and self._context.get('allowed_company_ids')[0] == 2:
                expanded_account = self.env['account.account'].browse(1191)
        else:
            raise UserError(_("Report not define."))
        ...
        for i, options_period in enumerate(options_list):
            ...
            queries.append('''
                SELECT
                    ...
                    'sum'                                                   AS key,
                    MAX(account_move_line.date_maturity)                    AS max_date,
                    %s                                                      AS period_number,
    ...
    def _get_query_amls(self, options, expanded_account, offset=None, limit=None):
        ...
        query = '''
            ...
            WHERE %s
            ORDER BY account_move_line.date_maturity, account_move_line.id
    ...
    def _get_aml_line(self, options, account, aml, cumulated_balance):
        ...
        columns = [
            {'name': format_date(self.env, aml['date_maturity'] if aml['date_maturity'] else aml['date']), 'class': 'date'},
